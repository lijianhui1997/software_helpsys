...
<script type="text/javascript">
	...
	function validateForm(){
		...
		$("#registerform").bootstrapValidator({
            ...
            fields: {
				...
				//实现验证码验证
				usercode : {
					messaage : 'The usercode is not valid',
					validators : {
						notEmpty : {
							message : '验证码不能为空'
						},						
						remote : {
							url : "CheckCode?action=check",//请求的路径
							type:"post",
	...
	function register() {
		// 异步注册用户	
		$.ajax({
			url : "userServlet?action=register",// 请求地址
			type : "POST", // 请求类型
			async : "true", // 是否异步方式
		...
		<form class="form-horizontal col-sm-offset-3" id="registerform" method="post">
		  ...
		  <!-- 注释掉   <div class="form-group">
			...
			<div class="form-group">
			  ...
			  <div>
			  	<img src="CreateCode" alt="验证码" id="num">
	         	<a href="javascript:;" onclick="document.getElementById('num').src='CreateCode?'+(new Date().getTime())">看不清换一张</a>
			  </div>
			</div>
